...
//insert
    ...
    // 새로 연동한 계좌는 사용자가 직접 선택하기 전까지 주계좌가 아니므로 STATUS를 NO로 저장
...
//update
    ...
    public String findIdByPhone(String phone) throws DataAccessException {
        ...
        // 아이디 찾기용 SQL: 휴대폰 번호가 일치하는 ACTIVE 회원의 id를 1건 조회
        String sql = """
                SELECT id
                FROM member_tb
                WHERE REPLACE(phone, '-', '') = ?
    ...
    public boolean existsActiveId(String id) throws DataAccessException {
        // 비밀번호 찾기용 SQL: 입력한 id가 ACTIVE 회원으로 존재하는지 확인
        String sql = """
                SELECT COUNT(*)
                FROM member_tb
                WHERE id = ?
    ...
    public boolean existsActiveMemberByIdAndPhone(String id, String phone) throws DataAccessException {
        // 비밀번호 찾기용 SQL: id와 휴대폰 번호가 같은 회원 정보인지 확인
        String normalizedPhone = normalizePhone(phone);
        String sql = """
                SELECT COUNT(*)
                FROM member_tb
    ...
    public void updatePasswordById(String id, String newPassword) throws DataAccessException {
        // 비밀번호 재설정용 SQL: 인증 완료된 id의 password와 updated_at을 갱신
        String sql = """
                UPDATE member_tb
                SET password = ?,
                    updated_at = SYSDATE
