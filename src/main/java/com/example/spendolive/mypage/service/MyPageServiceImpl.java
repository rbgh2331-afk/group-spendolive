...
public class MyPageServiceImpl implements MyPageService {
    ...
    // 사용자 본인 탈퇴는 활성 OTT 관계와 처리 중 환불이 모두 없을 때만 진행한다
    @Override
    @Transactional
    public MyPageDTO withdrawSelfMember(String loginId) throws Exception {
        ...
        // 동일 회원의 중복 탈퇴 요청을 막고 기존 member_id를 익명화 과정에 사용
        int memberId = myPageRepository.selectActiveMemberIdForUpdate(loginId);

        // 기존 OttService 조회 로직을 재사용해 가족방과 외부 모집방을 함께 검사한다
        ...
        // 제한 사유가 있으면 데이터 변경 없이 개수만 Controller에 반환
        if (!result.isWithdrawEligible()) {
            return result;
        }

        // 문의 DB 행을 삭제하기 전에 문의 번호를 보관해 실제 첨부파일 폴더도 함께 정리한다
        List<Integer> inquiryIdList = myPageRepository.selectInquiryIds(loginId);

        // 기존 아이디와 이메일을 비워 즉시 신규 회원가입에 사용 가능 상태로 처리
        ...
        // DB 트랜잭션이 정상 커밋된 뒤에만 문의 첨부파일 디렉토리를 삭제
        deleteInquiryFilesAfterCommit(inquiryIdList);
        return result;
    }

    // DB 롤백 시 파일만 먼저 사라지는 일을 막기 위해 커밋 이후 실제 파일을 정리한다
    private void deleteInquiryFilesAfterCommit(List<Integer> inquiryIdList) {
        ...
        // 트랜잭션 동기화가 없는 예외적인 실행 환경에서는 DB 처리 직후 파일을 정리한다
        deleteFiles.run();
    }

    // 임시 회원 아이디는 외래키 이동 중에만 사용하며 VARCHAR2(20)를 넘지 않게 한다
    ...
    // 최종 아이디 형식은 LEAVE_회원번호_기존아이디이며 전체 길이는 20자로 제한한다
