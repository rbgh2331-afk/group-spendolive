                ...
                <c:if test="${not empty inq.files}">
                    <div class="form-field"><label>첨부파일</label><div class="toolbar-left">
                        <c:forEach var="file" items="${inq.files}">
                            <c:choose>
                                <c:when test="${file.image}"><a href="${contextPath}/spendolive/inquiry/file/${file.file_id}" target="_blank" rel="noopener"><img src="${contextPath}/spendolive/inquiry/file/${file.file_id}" alt="${fn:escapeXml(file.origin_name)}" class="admin-inquiry-image"></a></c:when>
